...
/**
 * Get all index files in the vault matching the prefix.
 */
export function getIndexFiles(
	...
	return vault.getMarkdownFiles().filter(file => {
		if (!file.name.startsWith(prefix) || !file.name.endsWith(".md")) return false;
 ...
 */
export function isIndexFile(file: TFile, prefix: string): boolean {
...
export function getSiblingFiles(
	...
	return vault.getMarkdownFiles().filter(file => {
		...
		if (isIndexFile(file, prefix)) return false;
...
export function getNestedIndexFiles(
	...
	for (const subfolder of subfolders) {
		...
		const indexInSubfolder = vault.getMarkdownFiles().find(file => 
			getParentPath(file) === subfolder.path && isIndexFile(file, prefix)
		);
		if (indexInSubfolder) {
